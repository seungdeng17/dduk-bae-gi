...
public class CStageManager : MonoBehaviour {
    ...
    [Header("< 현재 스테이지 >")]
    public ObscuredFloat _stage = 1; // 현재 스테이지
    public Text _stageText;
    [HideInInspector]
    public int _clearStageNum = 0;
    ...
    public CBossMonsterManager _bossMonsterManager; // 보스 몬스터 리스폰
    ...
    // 몬스터 스탯 밸런스 세팅
    public void StageBalanceMonster(CMonsterState monsterState, ObscuredFloat stage)
    {
    ...
    // 코인 밸런스 세팅
    public void StageBalanceMoney(CCoin coin, ObscuredFloat stage)
    {
    ...
    public void StageClear()
    {
        ...
        _bossMonsterManager._bossMonsterNum -= 1;
        if (_bossMonsterManager._bossMonsterNum == (_bossMonsterManager._bossMonsterMaxNum - 1) && _bossMonsterManager.gameObject.activeSelf) // 보스 몬스터 매니저가 비활성화가 아니라면 (플레이어가 살아있다면)
        {
            _bossMonsterManager.BossMonsterSpawn(); // 보스 스폰
        }
    }
    ...
    public void StageTextRefresh()
    {
        ...
        CStringBuilder._sb.Append("STAGE <color=#FFFFFFC8>");
        CStringBuilder._sb.Append(_playerState.CommaText(_stage).ToString());
        CStringBuilder._sb.Append("</color>");
        _stageText.text = CStringBuilder._sb.ToString();
    }
}
